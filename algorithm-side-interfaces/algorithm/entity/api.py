...
def save_apis_to_json(apis):
    ...
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'user_api_list.json'), 'w') as f:
        json.dump(api_dicts, f, indent=4)


def load_apis_from_json():
    # 从 JSON 文件加载 API 对象数组
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'user_api_list.json'), 'r') as f:
