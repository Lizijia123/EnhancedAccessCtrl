...
class Brain:
    ...
    def gen_api_seq(self, malicious, role, action_step):
        auth_info = random.choice(self.auth_info_set[role])
        if malicious:
            question = (f"假设你是{auth_info}，你打破了权限检测机制，可以实现任意你想做的越权行为；作为恶意越权用户，你倾向于伪装成善意的用户。"
        ...
        try:
            ...
            self.logger.info(f"生成API序列: Question - {question}, Result - {result}")
            return result
        except Exception as e:
